...
contract HuskyCreator is ERC721, Ownable, SupportsInterfaceWithLookup, ERC721TokenReceiver, ERC721Holder, ERC721Metadata, UintToString {
    ...
    event SetToAuction(address indexed huskyOwner, uint indexed id);
    ...
    mapping (uint => bool) public toAuction;
    
    function setToAuction(uint id) public onlyOwnerOf(id){
        require(siring[id] == false);
        require(toAuction[id] == false);
        toAuction[id] = true;
        toAuctionCounter++;
        if(toSale[id] == true) {
            deleteSale(id);
        }
        emit SetToAuction(msg.sender, id);
    }
    
    function totalSupply() public view returns (uint) {
        return(toAuctionCounter);
    }   
    ...
    function getPuppy(uint _id) public view returns (bytes16, uint, bool, uint, bool, bool) {
        setVitalForce(_id, 0);
        return (pName[_id], vitalForce[_id], toSale[_id], salePrice[_id], siring[_id], toAuction[_id]);
    }
    ...
    function setToSale(uint _id, uint _price) public onlyOwnerOf(_id) {
        require(toSale[_id] == false && toAuction[_id] == false);
        require(_price != 0);
    ...
    function takePuppy(uint _id) public payable {
        address takePuppyOwner = puppyToOwner[_id];
        require(toSale[_id] == true && msg.value == salePrice[_id]);
        _transfer(takePuppyOwner, msg.sender, _id);
        toSale[_id] = false;
        puppiesToSale[idsSaleCounter[_id]] = puppiesToSale[toSaleCounter - 1];
        idsSaleCounter[puppiesToSale[toSaleCounter - 1]] = idsSaleCounter[_id];
        puppiesToSale[toSaleCounter - 1] = 0;
        ...
        ownersShare = 0;
        takePuppyOwner.transfer(toSend);
    }
